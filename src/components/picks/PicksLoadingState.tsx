...
import { Activity, Eye, Loader2, Newspaper, Sparkles, Target, TrendingUp } from 'lucide-react'
...
const SKELETON_ACCORDION_ROWS = [
  { icon: Target, width: 'w-[7.5rem]' },
  { icon: TrendingUp, width: 'w-[6.5rem]' },
  { icon: Activity, width: 'w-[5.5rem]' },
  { icon: Newspaper, width: 'w-[5rem]' },
  { icon: Eye, width: 'w-[8.5rem]' },
] as const
  ...
  return (
    ...
    >
      ...
      <div className="border-t border-amber-500/10 bg-zinc-950/35">
        {SKELETON_ACCORDION_ROWS.map(({ icon: Icon, width }, index) => (
          <div key={index} className="border-t border-white/[0.04] px-3.5 py-3 min-h-[48px]">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0 flex-1 space-y-2">
                <div className="flex items-center gap-1.5">
                  <Icon className="w-3.5 h-3.5 text-zinc-700 shrink-0" aria-hidden="true" />
                  <SkeletonBar className={cn('h-3', width)} delayMs={stagger + 240 + index * 30} />
                </div>
                <SkeletonBar className="h-3 w-[min(100%,14rem)]" delayMs={stagger + 260 + index * 30} />
              </div>
              <SkeletonBar className="h-4 w-4 rounded shrink-0 mt-0.5" delayMs={stagger + 280 + index * 30} />
            </div>
          </div>
        ))}
      </div>
